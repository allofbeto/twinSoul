...
const NewMonster = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        // Monsters are created via the campaign-scoped, owner-only endpoint, so
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.campaign_id) {
      setError('You need a campaign to create a monster in.');
      return;
    }
    setLoading(true);
    setError('');
    try {
      ...
      navigate(`/dashboard/monsters/${res.data.id}`);
    } catch (err: any) {
  ...
  return (
    <div>
      <h2 className="text-theme mb-1">New Monster</h2>
      <p className="text-muted-theme mb-4">Add a stat block to one of your campaigns.</p>
      ...
      {campaigns.length === 0 ? (
        <div className="card-theme p-4 text-center">
          <p className="text-muted-theme mb-0">
            You need to own a campaign before you can create a monster. Create one first.
          </p>
        </div>
      ) : (
